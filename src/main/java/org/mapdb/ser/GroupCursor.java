 ...
 * gap" for scan-heavy / columnar workloads: a caller that only wants to walk a value range —
 * or one column, via {@link ColumnarValueFormat#columnCursor} — reads bytes once, forward,
 * instead of calling {@link GroupFormat#binaryGet} per position (which for
 * delta/prefix/columnar layouts re-decodes from the group start each time, i.e. O(n²) for a
 * full scan).
 *
